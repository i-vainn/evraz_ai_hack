...
def generate_gas_features(df):
    ...
    volume_df = df[["NPLV"]].join(df[fraction_columns].multiply(df["V"] / 100, axis="index"))
    sum_volume_features = volume_df.groupby(by="NPLV").sum().rename(columns=lambda col : "gas_sum_volume_" + col)
    ...
    return pd.concat([duration_feature, mean_features, std_features, sum_volume_features, mean_volume_features, std_volume_features], axis=1)